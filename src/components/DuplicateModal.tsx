  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative bg-[#111111] rounded-3xl border border-white/10 shadow-2xl w-full max-w-lg overflow-hidden"
      >
        {/* Header */}
        <div className="flex items-center gap-3 px-5 py-4 border-b border-white/6">
          <GitMerge className="w-4 h-4 text-[#FF9F0A] shrink-0" />
          ...
          <button
            onClick={onClose}
            className="w-7 h-7 flex items-center justify-center rounded-lg bg-[#1C1C1E] border border-white/8 text-[#8E8E93] transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-5">
          {isDone ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm font-medium text-[#F5F5F7]">All duplicates resolved</p>
              <p className="text-xs text-[#48484A] mt-1">Your library is clean</p>
              <button
                onClick={onClose}
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                {group.series.map(s => {
                  ...
                  return (
                    <div key={s.id} className="bg-[#1C1C1E] rounded-2xl overflow-hidden border border-white/8">
                      <div className="aspect-[2/3] w-full relative">
                        {poster ? (
                          <img src={poster} alt={s.title} className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center p-3">
                            <span className="text-xs text-[#48484A] text-center">{s.title}</span>
                          </div>
                        )}
                      ...
                      <div className="p-3">
                        <p className="text-xs font-medium text-[#F5F5F7] leading-snug line-clamp-2 mb-1">{s.title}</p>
                        <p className="text-[11px] text-[#48484A] mb-3">
              ...
              {remaining.length > 1 && (
                <button
                  onClick={handleSkip}
                  className="w-full mt-3 py-2 text-[#48484A] hover:text-[#8E8E93] text-sm transition-colors"
                >
                  Skip for now
                </button>
              )}
