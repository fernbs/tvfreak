import { Home, Library, Search, BarChart2, Sparkles } from 'lucide-react'

export type Tab = 'home' | 'library' | 'search' | 'stats' | 'discover'
...
export function BottomNav({ active, onChange }: Props) {
  return (
    <nav
      className="shrink-0 w-full flex bg-black/94 backdrop-blur-2xl border-t border-white/6"
      style={{ paddingBottom: 'var(--nav-bleed, 0px)' }}
    >
      {tabs.map(({ id, icon: Icon, label }) => {
        ...
        return (
          ...
          >
            ...
            />
            <Icon
              className={`w-[22px] h-[22px] transition-all duration-200 ${isActive ? 'text-[var(--color-accent)]' : 'text-[#48484A]'}`}
              strokeWidth={isActive ? 2.5 : 1.8}
            />
            <span className={`text-[10px] font-medium tracking-wide transition-colors duration-200 ${
              isActive ? 'text-[var(--color-accent)]' : 'text-[#48484A]'
            }`}>
              {label}
            </span>
          </button>
